...
import { Matrix4, Vector3 as vec3, Vector2 } from '@math.gl/core'
import { bindMesh, drawMesh } from "./type/WebGLMesh";
import PlaneMesh from "./mesh/PlaneMesh";
import { loadTexture } from "./webGLUtils";
import { uniformProxy, UniformTypes } from "./type/WebGLUniformProxy";
...
export default {
  ...
  uniforms: [
    uniformProxy('u_sampleScale', UniformTypes.glFloat),
    uniformProxy('u_picture_size', UniformTypes.vector2),
    uniformProxy('u_sampleOffset', UniformTypes.vector2),
    uniformProxy('u_sampleRange', UniformTypes.glInt),
    // uniformProxy('u_pixelGrid', UniformTypes.vector2),
  ],
  ...
  //   if (sampleOffsetLoc)
  //     this.ctx.uniform3fv(sampleOffsetLoc, [.2, .2, 1])
  ...
  // },
  drawFrame() {
    if (!noiseTexture) return loadTexture(this.ctx!, '/textures/test2.png').then((t) => {
      noiseTexture = t.texture;
      this.uniforms[1]!.ref!.value =  new Vector2(t.w, t.h) //new Vector2(t.w, t.h)
    }) 
